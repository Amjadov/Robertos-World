...
static private var joysticks : Joystick_Script[];					// A static collection of all joysticks
...
private var DesignedScaleWidth : float = 800f;
private var DesignedScaleHeight : float = 480f;							
...
function Awake () {
    ...
    defaultRect.width = defaultRect.width * (Screen.width / DesignedScaleWidth);
	defaultRect.height = defaultRect.height * (Screen.height / DesignedScaleHeight);
	if (defaultRect.width > defaultRect.height)
	defaultRect.height = defaultRect.width;
	
	if (defaultRect.height > defaultRect.width)
...
function Update () {
	if (!enumeratedJoysticks) {
		...
		joysticks = FindObjectsOfType (Joystick_Script) as Joystick_Script[];
	...
	if (count == 0) {
		...
		for (var i : int = 0; i < count; i++) {
			...
			if (shouldLatchFinger && (lastFingerId == -1 || lastFingerId != touch.fingerId)) {
				...
				for (var j : Joystick_Script in joysticks) {
					if (j != null && j != this)
